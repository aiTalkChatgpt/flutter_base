...
import 'package:flutter_base/common/login/providers/auth.dart';
import 'package:flutter_base/common/login/providers/login_messages.dart';
import 'package:flutter_base/common/login/providers/login_theme.dart';
import 'package:flutter_base/common/login/regex.dart';
import 'package:flutter_base/common/login/theme.dart';
import 'package:flutter_base/common/login/widgets/auth_card.dart';
import 'package:flutter_base/common/login/widgets/fade_in.dart';
import 'package:flutter_base/common/login/widgets/gradient_box.dart';
import 'package:flutter_base/common/login/widgets/hero_text.dart';
import 'package:flutter_base/common/login/widgets/null_widget.dart';
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.titleTag != null && !DartHelper.isNullOrEmpty(widget.title)) {
      title = HeroText(
        ...
        style: widget.loginTheme.titleStyle,
...
class _FlutterLoginState extends State<FlutterLogin>
  ...
  void initState() {
    ...
    textEditingController = TextEditingController(text: BaseApi.BASE_URL);
  ...
  ThemeData _mergeTheme({ThemeData theme, LoginTheme loginTheme}) {
    ...
    final isDefaultBlackText = theme.textTheme.subtitle1.color ==
        Typography.blackMountainView.subtitle1.color;
    ...
    final textFieldStyle = theme.textTheme.headline6
  ...
  Widget build(BuildContext context) {
    ...
    return MultiProvider(
      ...
      child: Scaffold(
        ...
        body: Stack(
          children: <Widget>[
            if(BaseConstants.isUseWeather)
              WeatherBg(
                weatherType: WeatherType.values[BaseConstants.getWeatherBgIndex(context)],
                width: screenDpW,
                height: screenDpH,
              ),
            if(!BaseConstants.isUseWeather)
            ...
            Visibility(
              visible: BaseConstants.isDebug,
              child: _buildChangeBaseUrl(),
            ),
          ],
        ),
      ),
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          Container(
            child: FlatButton(child: Text('确定'), onPressed: () {
              BaseApi.BASE_URL = textEditingController.text.trim();
              setState(() {
