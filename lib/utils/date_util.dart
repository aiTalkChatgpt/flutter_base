...
class DateUtil {
  ...
  /// get Now Date Str.(yyyy-MM-dd HH:mm)
  static String getNowDateByYYYYMMDDHHMM() {
    return getDateStrByDateTime(DateTime.now(), format: DateFormat.YEAR_MONTH_DAY_HOUR_MINUTE);
  }

  /// get Now Date Str.(yyyy-MM-dd HH:mm:ss)
  static String getNowDateByYYYYMMDDHHMMSS() {
    return getDateStrByDateTime(DateTime.now(), format: DateFormat.NORMAL);
  }
  ...
  static String getNowDateByYYYYMM() {
    return getDateStrByDateTime(DateTime.now(), format: DateFormat.YEAR_MONTH);
  }
  ...
  /// 按照天为单位计算2个时间之间差
  static int difDate(String lastTime, String nowTime){
    int last = getDateMsByTimeStr(lastTime);
    int now = getDateMsByTimeStr(nowTime);
    int day = (now - last) ~/ (1000 * 60 * 60 * 24);
    return day + 1;
  }
}
