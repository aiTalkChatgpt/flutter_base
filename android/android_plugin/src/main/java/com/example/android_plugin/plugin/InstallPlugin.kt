package com.example.android_plugin.plugin
...
class InstallPlugin: FlutterPlugin {
    ...
    companion object {
        ...
        var flutterActivity : FlutterActivity? = null
    }

    private fun installApk(filePath: String?, appId: String?) {
        if (filePath == null) throw NullPointerException("fillPath is null!")
        val activity: Activity = registrar?.activity() ?: flutterActivity as Activity ?: throw NullPointerException("context is null!")
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        MultiLogUtil.iLog("InstallPlugin","onAttachedToEngine")
    ...
    fun setupChannel(messenger: BinaryMessenger, context: Context) {
        channel = MethodChannel(messenger, "install_plugin")
        channel?.setMethodCallHandler{ call, result ->
            MultiLogUtil.iLog("InstallPlugin","setMethodCallHandler:${call.method}")
            when (call.method) {
                "getPlatformVersion" -> {
